...
export function activate(context: vscode.ExtensionContext): void {
    ...
    vscode.commands.registerCommand('nimbus.initRepository', async () => {
      ...
      if (!workspace) {
        vscode.window.showWarningMessage(
          'Abra uma pasta ou repositório antes de inicializar o Nimbus Code Lite.'
        );
        return;
      }
      ...
      const files: ReadonlyArray<readonly [vscode.Uri, string]> = [
      ...
      try {
        ...
        vscode.window.showInformationMessage(
          'Nimbus Code Lite inicializado. Comece pela especificação da sua feature.'
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        vscode.window.showErrorMessage(
          `Não foi possível inicializar o Nimbus Code Lite: ${message}`
        );
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('nimbus.openLiteGuide', async () => {
      await vscode.env.openExternal(
        vscode.Uri.parse('https://venhapranuvem.com.br')
      );
    })
  );
}

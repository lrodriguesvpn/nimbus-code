...
if (cd "$WORKDIR" && "$SCRIPT_PATH" \
  ...
  if grep -q "REJEITADO" /tmp/audit_test_out_2.log \
     ...
     && grep -q -- "--exception-category" /tmp/audit_test_out_2.log; then
...
# --- Teste 4: gate de fechamento bloqueia com ajuste incompleto no período --
...
# --- Teste 5: gate de fechamento aprova período sem ajustes pendentes ------

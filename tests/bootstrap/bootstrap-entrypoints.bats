...
setup_file() {
  ...
  export TEST_ROOT="$REPO_ROOT/.bats-tmp/bootstrap-entrypoints"
  rm -rf "$TEST_ROOT"
  mkdir -p "$TEST_ROOT"
}
...
make_fake_toolchain() {
  ...
  cat > "$bindir/specify" <<'FAKE_SPECIFY'
#!/usr/bin/env bash
set -euo pipefail
...
@test "nimbus-code entrypoints do not use the broken raw GHE subdomain" {
  run bash -lc 'set -euo pipefail; cd "$1"; matches=$(grep -RIn "raw\\.venha-pra-nuvem\\.ghe\\.com/venha-pra-nuvem/nimbus-code/main/" README.md bootstrap.sh docs templates .github bundles presets extensions workflows tests || true); [ -z "$matches" ]' _ "$REPO_ROOT"
  [ "$status" -eq 0 ]
}
...
@test "bootstrap classifies a greenfield repo without application code" {
  ...
  workspace="$TEST_ROOT/greenfield"
...
@test "bootstrap classifies a brownfield repo with application code" {
  local workspace repo fake_bin
  workspace="$TEST_ROOT/brownfield"
...
@test "bootstrap fails when a critical component installation fails" {
  ...
  prepare_consumer_repo "$repo"
  mkdir -p "$fake_bin"
  cat > "$fake_bin/specify" <<'FAKE_SPECIFY_FAIL'
#!/usr/bin/env bash
set -euo pipefail
if [[ "${1:-}" == "init" ]]; then
  mkdir -p .specify

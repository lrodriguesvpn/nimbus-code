# Princípios Não-Negociáveis da Nimbus-Code — Plataforma/Cliente
...
<!--
  Este bloco é inserido pelo preset `nimbus-code-platform-standards` (estratégia `wrap`).
  ...
  `nimbus-code-standards`. Os dois presets são mutuamente exclusivos por
  ...
  aprovação do time responsável pelos padrões da Nimbus-Code e segue o controle de
  versão do preset (ver presets/nimbus-code-platform-standards/preset.yml).
-->
...
> Este repositório só deve ser criado depois que os dois presets (`nimbus-code-standards`
> e `nimbus-code-platform-standards`) estiverem estáveis e publicados no catálogo —
...
- **Toda mudança REAL em qualquer ambiente nasce em um repositório de PROJETO**
  (workload), usando o preset `nimbus-code-standards`, nunca aqui. Quando não existe
...
- Violação desta regra (qualquer PR/workflow que introduza um passo de
  ...
  bloqueia merge e exige revisão do @nimbus-code-arch-board.
...
- A Nimbus-Code opera hoje sobre **Azure, AWS, GCP, Google Workspace (GWS),
...
- **Mudança de schema nunca acontece neste repositório** — apenas
  ...
  aquele sistema, seguindo `nimbus-code-standards`.
...
- Todo repositório de **workload** (`nimbus-code-standards`) que consome uma
...
- Reutiliza a taxonomia base do `nimbus-code-standards`
...
- `type:incident` (Ocorrência CRM N1 → issue de infraestrutura) segue a mesma
  regra do `nimbus-code-standards`: sempre bloqueia autonomia, independente de
  complexidade.

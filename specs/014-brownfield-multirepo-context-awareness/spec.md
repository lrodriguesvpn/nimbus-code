...
- **FR-004**: O script `harvest-patterns.sh <repo-path> [--subpath <dir>] [--output <arquivo>]` DEVE extrair metadados estruturais do repo (nomes de arquivo, assinaturas de método/interface/classe, anotações/decoradores — **nunca** o corpo de métodos, strings literais ou dados de runtime) e enviar apenas esses metadados para um LLM via API (ex.: Copilot API / OpenAI-compatible endpoint), solicitando a identificação de padrões arquiteturais reutilizáveis e produzindo entradas no formato `reuse-catalog.yaml`. O endpoint e o token de autenticação são configuráveis via variáveis de ambiente (`HARVEST_API_URL`, `HARVEST_API_TOKEN`).

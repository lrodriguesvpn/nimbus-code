...
- [x] T018 Adicionar um gate de validação pós-execução incondicional e determinístico em `.github/skills/speckit-taskstoissues/SKILL.md` (com um script auxiliar correspondente, no mesmo padrão de `.specify/scripts/bash/check-epic-issue-consistency.sh`) que confirme, antes de reportar conclusão, que toda issue de User Story e Task processada na execução atual está de fato vinculada ao parent correto no GHE (consultando `parent`/`subIssuesSummary` via GraphQL) — em vez de depender apenas da prosa do Outline sendo seguida corretamente pelo agente per FR-009, reuse-catalog tag `skill-mid-flow-instruction-reliability-gate`
  - **Nota**: implementado via novo script `.specify/scripts/bash/check-task-hierarchy-consistency.sh` (mesmo padrão de `check-epic-issue-consistency.sh`: bash puro, `--json`, `--dry-run`, idempotente, no-op com lista vazia) + nova seção "Mandatory Post-Execution Validation (Task Hierarchy Consistency)" no `SKILL.md`, chamada incondicionalmente ao final do Outline, uma vez por repositório-alvo (multirepo-aware). Autocorrige delegando para `create-github-issue-hierarchy.sh link-task` (sem duplicar a mutation `addSubIssue`) e nunca engole silenciosamente um par que permaneça inconsistente após a tentativa de correção (exit code 1 + relatório ao usuário). Reuse-catalog `skill-mid-flow-instruction-reliability-gate` atualizado (reuse_count 2→3) com esta nova aplicação do padrão.

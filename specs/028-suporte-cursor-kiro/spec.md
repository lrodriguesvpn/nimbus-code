...
- **Achado pós-implementação (HRN-0008)**: a validação original desta spec cobriu apenas o repositório-template (testes bats + instalação manual local), sem exercitar o caminho real de propagação para satélites (`bootstrap.sh`). O `case "$INTEGRATION"` de `bootstrap.sh` que decide o alvo de sync de agentes NC-* nunca foi atualizado com `cursor-agent`/`kiro-cli`, deixando repositórios satélite configurados com essas integrações sem receber os agentes NC-* nem os comandos `/speckit-*` proprietários. Corrigido em `bootstrap.sh` (adição dos casos `cursor-agent|cursor` e `kiro-cli|kiro`) com teste de regressão dedicado (`bootstrap-nc-agent-sync-coverage.bats`) que verifica dinamicamente que todo alvo em `TARGETS` do gerador está representado no dispatch de `bootstrap.sh`.

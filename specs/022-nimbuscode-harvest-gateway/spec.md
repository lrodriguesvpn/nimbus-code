...
- Repositório próprio (`venha-pra-nuvem/nimbus-harvest-gateway`) será criado para hospedar a implementação; esta spec vive no repositório-fonte (`nimbus-code`) apenas como artefato de planejamento, seguindo o mesmo precedente de `vpn-skills` (SPEC-003).

...
| AC-2 | `docs/reuse-catalog.yaml` existe e é machine-readable (parseável como YAML) | Validação estrutural (poderia ser automatizada futuramente) | `docs/reuse-catalog.yaml` | Nesta fase, validado manualmente (parse YAML bem-sucedido verificado em `research.md`); `.github/workflows/validate-manifests.yml` já cobre outros manifests do bundle e poderia ser estendido para este arquivo numa iteração futura (fora de escopo aqui). Caminho corrigido em 2026-09-20; o catálogo e seu template também foram parseados como YAML na auditoria local dessa data. |

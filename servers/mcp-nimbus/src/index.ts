...
const server = new Server(
  { name: 'nimbus-code-mcp-lite', version: '0.1.0' },
  { capabilities: { tools: {} } }
);
  ...
  tools: [
    {
      ...
      description:
        'Retorna um template Lite local para constituicao, especificacao, plano ou tarefas.',
      inputSchema: {
        type: 'object',
        properties: {
          artifact: {
    ...
    },
    {
      ...
      description:
        'Explica o fluxo Lite local de especificacao, planejamento e validacao.',
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      content: [
        {
          type: 'text',
          text:
            'Comece descrevendo a feature, registre a especificacao, planeje a menor mudanca completa e execute a validacao correspondente.',
        },
      ],
    };
  }
  ...
  if (request.params.name === 'nimbus_lite_get_template') {
    ...
    if (typeof artifact !== 'string' || !(artifact in templates)) {
      throw new Error(
        'artifact deve ser constitution, spec, plan ou tasks.'
      );
    }
    return {
      content: [{ type: 'text', text: templates[artifact as TemplateName] }],
    };
  }
